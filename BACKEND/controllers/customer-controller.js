...
const updateCustomer = async(req,res,next)=>{
    ...
    const {CustomerName, CustomerEmail, ContactNum, Address, CusUsername} = req.body;
    ...
    .then(()=>{
        ...
        console.log(err);
        res.status(500).send({status:"Error in updating customer", error: err.message})
    })
}
...
const deleteCustomer = async(req,res,next)=>{
    ...
    .then(()=>{
        ...
        res.status(500).send({status: "Error in deleting customer", error: err.message});
    })
}
